...
from input_models.models import OspfQuery, BgpQuery, InterfacesQuery
...
async def _collect_device(device: str, queries: set[str]) -> DeviceState:
    ...
    if "interfaces" in queries:
        result = await get_interfaces(InterfacesQuery(device=device, transport="ssh"))
        if "error" in result:
    ...
    if "ospf_neighbors" in queries:
        result = await get_ospf(OspfQuery(device=device, query="neighbors", transport="ssh"))
        if "error" in result:
    ...
    if "ospf_details" in queries:
        result = await get_ospf(OspfQuery(device=device, query="details", transport="ssh"))
        if "error" in result:
            ...
            config_result = await get_ospf(OspfQuery(device=device, query="config", transport="ssh"))
    ...
    if "bgp_summary" in queries:
        result = await get_bgp(BgpQuery(device=device, query="summary", transport="ssh"))
        if "error" in result:
